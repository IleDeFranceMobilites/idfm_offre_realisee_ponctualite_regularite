...
def file_system_fixture():
    ...
    os.remove(os.path.join(TEST_DATA_PATH, TEST_DATA_PATH_CONFIG['output_path'],
                           ...
                           f"mesure_{MesureType.ponctualite}_{suffix}" + FileExtensions.csv))
    os.remove(os.path.join(TEST_DATA_PATH, TEST_DATA_PATH_CONFIG['output_path'],
...
def test_create_mesure_qs(file_system_fixture):
    ...
    # When
    create_mesure_qs(local_file_system_handler, date)
    ...
    result_clean = os.listdir(
        os.path.join(
            TEST_DATA_PATH, TEST_DATA_PATH_CONFIG['output_path'], AggregationLevel.by_day, MesureType.ponctualite
        )
    )
